...
from pia import run_proximal_inference_attack
...
def train(config, workdir):
    ...
    elif config.sde == 'hold':
        print('HOLD', flush=True)
        sde = sde_lib.HOLD(config)
    else:
    ...
    while step < num_total_iter:
        for _, (train_x, _) in enumerate(train_queue):
            ...
            if step % config.snapshot_freq == 0 and global_rank == 0 and step >= config.snapshot_threshold:
                ...
                ## Perform PIA here:
                print('Before PIA', flush=True)
                run_proximal_inference_attack(config, score_model, sde)
                print('After PIA', flush=True)
                ##
                ema.store(score_model.parameters())
                ema.copy_to(score_model.parameters())
                u = sampling_fn(score_model)
                x = torch.chunk(u, config.model_order, dim=1)[0]

                ema.restore(score_model.parameters())

                x = inverse_scaler(x)
                logging.info('NFE for snapshot at step %d: %d' % (step, 0))
                writer.add_scalar('nfe', 0, step)
...
def evaluate(config, workdir):
    ...
    ## Run Diff privacy stuff before FID:
    print('Before PIA', flush=True)
    run_proximal_inference_attack(config, score_model, sde)
    print('After PIA', flush=True)
    exit()
