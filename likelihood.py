...
def get_ode_likelihood_fn(config, sde):
    ...
    def likelihood_fn(model, data):
        ...
        with torch.no_grad():
            ...
            if sde.is_hold:
                t_tensor = torch.tensor([config.likelihood_eps, hold_T], device=config.device)
            else:
                t_tensor = torch.tensor([config.likelihood_eps, 1.], device=config.device)

            #print('len(t_tensor) {}'.format(t_tensor))
            #print('likelihood_rtol {}'.format(config.likelihood_rtol))
            #print('likelihood_atol {}'.format(config.likelihood_atol))
            #print('likelihood_solver {}'.format(config.likelihood_solver))
            #print('likelihood_solver_options {}'.format(config.likelihood_solver_options))
            ...
            if sde.is_hold:
                prior_logpx, prior_logpy, prior_logpz = sde.prior_logp(u_T)
                nll = -(prior_logpx + prior_logpy + prior_logpz + delta_logp)
            elif sde.is_augmented:
