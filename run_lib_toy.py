...
def train(config, workdir):
    ...
    while step < num_total_iter:
        ...
        if (step % config.snapshot_freq == 0 or step == num_total_iter) and global_rank == 0 and step >= config.snapshot_threshold:
            ...
            ema.store(score_model.parameters())
            ema.copy_to(score_model.parameters())
            q, p, s, nfe = sampling_fn(score_model)
#            x, v, nfe = sampling_fn(score_model)
            ...
            plt.scatter(q.cpu().numpy()[:, 0], q.cpu().numpy()[:, 1], s=3)
            plt.savefig(os.path.join(this_sample_dir,
                        'sample_rank_%d.png' % global_rank))
            plt.close()

            if config.sde == 'cld':
                np.save(os.path.join(this_sample_dir, 'sample_q'), q.cpu())
                np.save(os.path.join(this_sample_dir, 'sample_p'), p.cpu())
                np.save(os.path.join(this_sample_dir, 'sample_s'), s.cpu())
            else:
                np.save(os.path.join(this_sample_dir, 'sample'), q.cpu())
        dist.barrier()
