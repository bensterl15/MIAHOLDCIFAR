...
def get_ode_sampler(config, sde, sampling_shape, eps):
    ...
    def ode_sampler(model, u=None):
        with torch.no_grad():
            if u is None:
                if sde.is_hold:
                    q, p, s = sde.prior_sampling(sampling_shape)
                    u = torch.cat((q, p, s), dim=1)
                else:
            ...
            if sde.is_hold:
                time_tensor = torch.tensor(
                    [0., hold_T - eps], dtype=torch.float64, device=config.device)
            else:
                time_tensor = torch.tensor(
                    [0., 1. - eps], dtype=torch.float64, device=config.device)
            #print('len(time_tensor) {}'.format(len(time_tensor)))
            ...
            if config.denoising:
                if sde.is_hold:
                    u = denoising_fn(model, u, hold_T - eps)
                else:
...
def get_em_sampler(config, sde, sampling_shape, eps):
    ...
    def em_sampler(model, u=None):
        with torch.no_grad():
            if u is None:
                u = sde.prior_sampling(sampling_shape)
            ...
            if config.denoising:
                if sde.is_hold:
                    _, u = step_fn(model, u, hold_T - eps, eps)
                else:
